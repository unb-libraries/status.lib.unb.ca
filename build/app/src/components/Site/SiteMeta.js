    ...
    <React.Fragment>
      <Inline itemClassName={classes.meta}>
        <Inline>
          ...
          <span>{props.pages.length} page{props.pages.length !== 1 && 's'}</span>
        </Inline>
        <Inline>
          <Icon icon={Icons.clock}/>
          <DateTimestamp 
            milliseconds={props.timestamp} 
            elapsed={true} 
            text={'{} ago'} 
            altText={'Less than a minute'} 
            tooltip={'Last tested on {}'} />
        </Inline>
        <Inline>
          <Icon icon={props.stats.unresolvedErrors > 0 ? Icons.arrowDown : Icons.arrowUp} />
          {props.stats.unresolvedErrors > 0
            ? <DateTimestamp 
                milliseconds={props.stats.failingSince} 
                elapsed={true} 
                text={`Failing for ${props.stats.unresolvedErrors === 0 ? 'at least ' : ''}{}`} 
                altText={'Less than a minute'} 
                tooltip={'Failing since {}'} />
            : <DateTimestamp 
                milliseconds={props.stats.passingSince} 
                elapsed={true} 
                text={`No errors for ${props.stats.resolvedErrors === 0 ? 'at least ' : ''}{}`} 
                altText={'Less than a minute'} 
                tooltip={'No errors since {}'} />
          }
        </Inline>
      </Inline>
      {!props.collapsed && (
